...
@echo off
rem API Server startup script
rem This script starts, stops, and restarts the API server using Java
...
if exist "%PID_FILE%" (
    for /f %%p in ('type "%PID_FILE%"') do (
        ...
        if not errorlevel 1 (
            echo API Server is already running as process %%p
            exit /b 0
        )
    )
)
...
if defined SERVER_PID (
    ...
    echo API Server started successfully with process ID %SERVER_PID%
) else (
...
if exist "%PID_FILE%" (
    for /f %%p in ('type "%PID_FILE%"') do (
        ...
        tasklist /FI "PID eq %%p" 2>nul | find /I "java.exe" >nul
        if not errorlevel 1 (
            echo API Server is running ^(PID: %%p^)
        ) else (
            echo API Server is not running
            del "%PID_FILE%" 2>nul
        )
